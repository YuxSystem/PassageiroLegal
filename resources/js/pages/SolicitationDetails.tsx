...
const SolicitationDetails = ({ solicitation, agents }: Props) => {
  ...
  const { whatsapp } = usePage().props as any;
  ...
  const handleAssign = () => {
    ...
    router.post(
      `/admin/solicitacao/${solicitation.id}/atribuir`,
      {
  ...
  const handleWhatsAppContact = () => {
    const WHATSAPP_NUMBER = whatsapp?.number;
    if (!WHATSAPP_NUMBER) {
      console.error(
        "Número do WhatsApp não configurado. Configure a variável WHATSAPP_NUMBER no arquivo .env"
      );
      return;
    }
    const user = solicitation.user;
    const userName = user?.name || "Cliente";
    const userEmail = user?.email || "";
    const userPhone = user?.cellphone || "";
    const userDocument = user?.legal_document || "";

    // Monta a mensagem com os dados da solicitação
    let message = `Olá! Preciso de atendimento sobre minha solicitação.\n\n`;
    message += `*Dados da Solicitação:*\n`;
    message += `• ID: ${solicitation.id.slice(0, 8)}...\n`;
    message += `• Status: ${getStatusLabel(solicitation.status)}\n`;
    message += `• Motivo: ${solicitation.motivo || "Não informado"}\n`;

    if (solicitation.num_voo) {
      message += `• Número do Voo: ${solicitation.num_voo}\n`;
    }

    if (solicitation.dta_voo) {
      try {
        message += `• Data do Voo: ${format(
          new Date(solicitation.dta_voo),
          "dd/MM/yyyy",
          {
            locale: dateLocale,
          }
        ...
        // Se houver erro ao formatar a data, ignora
      }
    }

    if (solicitation.detalhe) {
      message += `• Detalhes: ${solicitation.detalhe}\n`;
    }
    ...
    if (userDocument) {
      message += `• Documento: ${userDocument}\n`;
    }

    message += `\nPor favor, entre em contato para dar continuidade ao processo.`;
    ...
    <>
      ...
      <div className="px-2 sm:px-4 md:container mx-auto py-6 md:py-10">
        ...
        <div className="mt-4 md:mt-6 space-y-4 md:space-y-6">
          {!isAdmin && solicitation.status.toLowerCase() === "finalizado" && (
            <Card className="border-green-200 bg-green-50">
              ...
              <CardContent>
                <p className="text-sm text-gray-700 mb-4">
                  Sua solicitação foi finalizada. Entre em contato conosco via
                  WhatsApp para dar continuidade ao processo.
                </p>
                <Button
                  onClick={handleWhatsAppContact}
                  className="bg-green-600 hover:bg-green-700 text-white"
          ...
          <Card>
            ...
            <CardContent>
              <div>
                <p className="text-sm font-medium text-gray-500">
                  {t("solicitations.details")}
                </p>
                <p className="mt-1 whitespace-pre-wrap">
                  {solicitation.detalhe}
                </p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="grid gap-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <p className="text-sm font-medium text-gray-500">
                    {t("solicitations.createdAt")}
                  </p>
                  <p className="mt-1">
                    {format(
                      new Date(solicitation.created_at),
                      "dd/MM/yyyy HH:mm",
                      {
                        locale: dateLocale,
                      }
                ...
                <div>
                  <p className="text-sm font-medium text-gray-500">
                    {t("solicitations.updatedAt")}
                  </p>
                  <p className="mt-1">
                    {format(
                      new Date(solicitation.updated_at),
                      "dd/MM/yyyy HH:mm",
                      {
                        locale: dateLocale,
                      }
              ...
              <CardContent>
                {solicitation.assigned_to ? (
                  <div className="space-y-2">
                    <div>
                      <p className="text-sm font-medium text-gray-500">
                        {t("solicitations.assignedTo")}
                      </p>
                      <p className="mt-1">
                        {solicitation.assigned_to_user?.name || "N/A"}
                      </p>
                    </div>
                    {solicitation.assigned_at && (
                      <div>
                        <p className="text-sm font-medium text-gray-500">
                          {t("solicitations.assignedAt")}
                        </p>
                        <p className="mt-1">
                          {format(
                            new Date(solicitation.assigned_at),
                            "dd/MM/yyyy HH:mm",
                            {
                              locale: dateLocale,
                            }
                    ...
                    {solicitation.assigned_by_user && (
                      <div>
                        <p className="text-sm font-medium text-gray-500">
                          {t("solicitations.assignedBy")}
                        </p>
                        <p className="mt-1">
                          {solicitation.assigned_by_user.name}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  <p className="text-sm text-gray-500">
                    {t("solicitations.noAgentAssigned")}
                  </p>
                )}
              </CardContent>
            </Card>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  {(!solicitation.validation_status ||
                    solicitation.validation_status === "Pendente") && (
                    <Button
                      variant="outline"
                      size="sm"
              ...
              <CardContent>
                {solicitation.validation_status ? (
                  <div className="space-y-2">
                    <div>
                      <p className="text-sm font-medium text-gray-500">
                        {t("solicitations.validationStatus")}
                      </p>
                      <Badge
                        className={
                          solicitation.validation_status === "Aprovado"
                    ...
                    {solicitation.validated_by_user && (
                      <div>
                        <p className="text-sm font-medium text-gray-500">
                          {t("solicitations.validatedBy")}
                        </p>
                        <p className="mt-1">
                          {solicitation.validated_by_user.name}
                        </p>
                      </div>
                    )}
                    {solicitation.validated_at && (
                      <div>
                        <p className="text-sm font-medium text-gray-500">
                          {t("solicitations.validatedAt")}
                        </p>
                        <p className="mt-1">
                          {format(
                            new Date(solicitation.validated_at),
                            "dd/MM/yyyy HH:mm",
                            {
                              locale: dateLocale,
                            }
                  ...
                  <p className="text-sm text-gray-500">
                    {t("solicitations.pendingValidation")}
                  </p>
                )}
              </CardContent>
            </Card>
                    ...
                    >
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-gray-600 mt-1">
                          {t("common.by")}{" "}
                          <strong>{history.changed_by.name}</strong>
                        </p>
        ...
        <DialogContent>
          ...
          <div className="space-y-4 py-4">
            <Select
              ...
              onValueChange={(value) =>
                setValidationData("status", value as "Aprovado" | "Rejeitado")
              }
            >
              <SelectTrigger>
              ...
              <SelectContent>
                <SelectItem value="Aprovado">
                  {t("solicitations.approve")}
                </SelectItem>
                <SelectItem value="Rejeitado">
                  {t("solicitations.reject")}
                </SelectItem>
              </SelectContent>
            </Select>
            <Textarea
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className={
                validationData.status === "Aprovado"
